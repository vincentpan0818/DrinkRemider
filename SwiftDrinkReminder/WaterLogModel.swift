...
final class WaterLogModel: ObservableObject {
  ...
  init(defaults: UserDefaults = .standard, calendar: Calendar = .current) {
    ...
    reminderFrequency =
      ReminderFrequency(
        rawValue: defaults.string(forKey: reminderFrequencyKey) ?? ReminderFrequency.smart.rawValue)
      ?? .smart
  ...
  private func upcomingReminderDates(
    ...
    for dayOffset in 0..<7 {
      guard dates.count < maxCount,
        let currentDayStart = calendar.date(byAdding: .day, value: dayOffset, to: dayStart)
      else {
        break
      }
      ...
      for minuteOfDay in dayTimes {
        ...
        guard
          let fireDate = calendar.date(byAdding: .minute, value: minuteOfDay, to: currentDayStart)
        else {
          continue
        }
        if fireDate > now {
  ...
  private func saveToHealthKit(entry: DrinkEntry) async {
    ...
    let sample = HKQuantitySample(
      type: waterType, quantity: quantity, start: entry.timestamp, end: entry.timestamp)
