...
enum AdMobConfiguration {
  ...
  static var isConfigured: Bool {
    !appID.isEmpty && !bannerAdUnitID.isEmpty && !appID.contains("YOUR_")
...
private struct AdMobBannerRepresentable: UIViewControllerRepresentable {
  ...
  final class Coordinator: NSObject, BannerViewDelegate {
    ...
    func bannerView(_ bannerView: BannerView, didFailToReceiveAdWithError error: any Error) {
      print(
        "[AdMob] Banner failed: \(bannerView.adUnitID ?? "unknown") | \(error.localizedDescription)"
      )
    }
...
private final class BannerHostingController: UIViewController {
  ...
  func updateBannerSize() {
    ...
    if bannerView.adSize.size.width != adSize.size.width
      || bannerView.adSize.size.height != adSize.size.height
    {
